...
def cornersHeuristic(state, problem):
  ...
  while len(unvisitedCorners) > 0:
    ...
    sum += len(unvisitedCorners) * distance #enforce going towards the closest corner always
    currentPoint = corner
...
def foodHeuristic(state, problem):
  ...
  total = len(foodGridList)
  while len(foodGridList) > 0:
    distance, food = min([(util.manhattanDistance(currentPoint, food), food) for food in foodGridList])
    #print 'distance: ',distance
    sum += (distance - 1) #only count empty strolls
    if (distance >= 1): sum += len(foodGridList) #punish empty strolls more when many food avail
    currentPoint = food
    foodGridList.remove(food)
  score = sum
  print "Heuristic score: ", score

  """Matrix rain"""
  items = [unichr(i) for i in range(50 , 100)]
  #items = [unichr(i) for i in range(0x30a1,0x30ff + 1)]  #katakana
